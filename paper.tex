...
Double Robotics\cite{Double}のDoubleは遠隔地から操作可能な高さ約1.2mのテレプレゼンス
...
動画の配信にはAndroidアプリケーションIP Webcam\cite{ipwebcam}を使用し，MotionJPEG形式の
影像をHTTP通信で配信している．
...
操作はLindaという並列計算プリミティブをWebサーバ上に実装したlinda-server\cite{linda}を
用いて実装している．
